...
export default function BlogPost(){
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-10 section-end page-shell">
      <Reveal>
        <Link to="/blog" className="inline-flex items-center gap-2 text-indigo-400 hover:text-indigo-300 text-sm mb-8 link-hover-lift">
          <ArrowLeft className="w-4 h-4"/> Back to Articles
        </Link>
      </Reveal>
      {post.cover_image_url && (
        <Reveal variant="fade" delay={60}>
          <img src={post.cover_image_url} alt="cover" className="w-full rounded-2xl mb-8 max-h-72 object-cover"/>
        </Reveal>
      )}
      <Reveal delay={100}>
